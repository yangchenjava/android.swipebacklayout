...
package com.yangc.swipebacklayout.helper;
...
/**
 * ViewDragHelper is a utility class for writing custom ViewGroups. It offers a number of useful operations and state tracking for allowing a user to drag and reposition views within their parent
 * ViewGroup.
 */
public class ViewDragHelper {
	...
	/**
	 * A Callback is used as a communication channel with the ViewDragHelper back to the parent view using it. <code>on*</code>methods are invoked on siginficant events and several accessor methods
	 * are expected to provide the ViewDragHelper with more information about the state of the parent view upon request. The callback also makes decisions governing the range and draggability of child
	 * views.
	 */
	public static abstract class Callback {
		...
		/**
		 * Called when a child view is captured for dragging or settling. The ID of the pointer currently dragging the captured view is supplied. If activePointerId is identified as
		 * {@link #INVALID_POINTER} the capture is programmatic instead of pointer-initiated.
		 *
		 ...
		 * <p>
		 * Calling code may decide to fling or otherwise release the view to let it settle into place. It should do so using {@link #settleCapturedViewAt(int, int)} or
		 * {@link #flingCapturedView(int, int, int, int)}. If the Callback invokes one of these methods, the ViewDragHelper will enter {@link #STATE_SETTLING} and the view capture will not fully end
		 * until it comes to a complete stop. If neither of these methods is invoked before <code>onViewReleased</code> returns, the view will stop in place and the ViewDragHelper will return to
		 * {@link #STATE_IDLE}.
		 * </p>
		 *
		...
		/**
		 * Called when the given edge may become locked. This can happen if an edge drag was preliminarily rejected before beginning, but after {@link #onEdgeTouched(int, int)} was called. This method
		 * should return true to lock this edge or false to leave it unlocked. The default behavior is to leave edges unlocked.
		 *
		...
		/**
		 * Called when the user's input indicates that they want to capture the given child view with the pointer indicated by pointerId. The callback should return true if the user is permitted to
		 * drag the given view with the indicated pointer.
		 * <p>
		...
		/**
		 * Restrict the motion of the dragged child view along the horizontal axis. The default implementation does not allow horizontal motion; the extending class must override this method and
		 * provide the desired clamping.
		 *
		...
		/**
		 * Restrict the motion of the dragged child view along the vertical axis. The default implementation does not allow vertical motion; the extending class must override this method and provide
	 ...
	 * @param sensitivity value between 0 and 1, the final value for touchSlop = ViewConfiguration.getScaledTouchSlop * (1 / s);
	 */
	...
	/**
	 * Return the currently configured minimum velocity. Any flings with a magnitude less than this value in pixels per second. Callback methods accepting a velocity will receive zero as a velocity
	 * value if the real detected velocity was below this threshold.
	 *
	...
	/**
	 * Enable edge tracking for the selected edges of the parent view. The callback's {@link me.imid.swipebacklayout.lib.ViewDragHelper.Callback#onEdgeTouched(int, int)} and
	 * {@link me.imid.swipebacklayout.lib.ViewDragHelper.Callback#onEdgeDragStarted(int, int)} methods will only be invoked for edges for which edge tracking has been enabled.
	 *
	...
	/**
	 * Set the size of an edge. This is the range in pixels along the edges of this view that will actively detect edge touches or drags if edge tracking is enabled.
	 *
	 ...
	 * {@link me.imid.swipebacklayout.lib.ViewDragHelper.Callback#tryCaptureView(android.view.View, int)} will not be asked permission to capture this view.
	 *
	...
	/**
	 * Animate the view <code>child</code> to the given (left, top) position. If this method returns true, the caller should invoke {@link #continueSettling(boolean)} on each subsequent frame to
	 * continue the motion until it returns false. If this method returns false there is no further work to do to complete the movement.
	 * <p>
	...
	/**
	 * Settle the captured view at the given (left, top) position. The appropriate velocity from prior motion will be taken into account. If this method returns true, the caller should invoke
	 * {@link #continueSettling(boolean)} on each subsequent frame to continue the motion until it returns false. If this method returns false there is no further work to do to complete the movement.
	 *
	...
	/**
	 * Settle the captured view based on standard free-moving fling behavior. The caller should invoke {@link #continueSettling(boolean)} on each subsequent frame to continue the motion until it
	 * returns false.
	 *
	...
	public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
		...
		mScroller.fling(mCapturedView.getLeft(), mCapturedView.getTop(), (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId),
	...
	/**
	 * Move the captured settling view by the appropriate amount for the current time. If <code>continueSettling</code> returns true, the caller should call it again on the next frame to continue.
	 *
	 * @param deferCallbacks true if state callbacks should be deferred via posted message. Set this to true if you are calling this method from {@link android.view.View#computeScroll()} or similar
	 ...
	 * {@link #settleCapturedViewAt(int, int)} or {@link #flingCapturedView(int, int, int, int)}.
	 */
	 ...
	 * <p>
	 * The state used to report this information is populated by the methods {@link #shouldInterceptTouchEvent(android.view.MotionEvent)} or {@link #processTouchEvent(android.view.MotionEvent)}. If
	 * one of these methods has not been called for all relevant MotionEvents to track, the information reported by this method may be stale or incorrect.
	 * </p>
	 *
	...
	/**
	 * Attempt to capture the view with the given pointer ID. The callback will be involved. This will put us into the "dragging" state. If we've already captured this view with this pointer this
	 * method will immediately return true without consulting the callback.
	 *
	...
	private boolean checkNewEdgeDrag(float delta, float odelta, int pointerId, int edge) {
		...
		if ((mInitialEdgeTouched[pointerId] & edge) != edge || (mTrackingEdges & edge) == 0 || (mEdgeDragsLocked[pointerId] & edge) == edge || (mEdgeDragsInProgress[pointerId] & edge) == edge
	...
	/**
	 * Check if we've crossed a reasonable touch slop for the given child view. If the child cannot be dragged along the horizontal or vertical axis, motion along that axis will not count toward the
	 * slop check.
	 *
	 ...
	 * <p>
	 * This depends on internal state populated by {@link #shouldInterceptTouchEvent(android.view.MotionEvent)} or {@link #processTouchEvent(android.view.MotionEvent)}. You should only rely on the
	 * results of this method after all currently available touch data has been provided to one of these two methods.
	 * </p>
	 *
	 * @param directions Combination of direction flags, see {@link #DIRECTION_HORIZONTAL}, {@link #DIRECTION_VERTICAL}, {@link #DIRECTION_ALL}
	 ...
	 *
	 * @param edges Edges to check for an initial edge touch. See {@link #EDGE_LEFT}, {@link #EDGE_TOP}, {@link #EDGE_RIGHT}, {@link #EDGE_BOTTOM} and {@link #EDGE_ALL}
	...
	/**
	 * Check if any of the edges specified were initially touched by the pointer with the specified ID. If there is no currently active gesture or if there is no pointer with the given ID currently
	 * down this method will return false.
	 *
	 * @param edges Edges to check for an initial edge touch. See {@link #EDGE_LEFT}, {@link #EDGE_TOP}, {@link #EDGE_RIGHT}, {@link #EDGE_BOTTOM} and {@link #EDGE_ALL}
	 * @return true if any of the edges specified were initially touched in the current gesture
	 */
